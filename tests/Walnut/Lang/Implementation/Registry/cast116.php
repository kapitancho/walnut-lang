...
$addVariable('mutate', $function($Mutable($String()), $Null(), $functionBody(
	$sequence([
		$method($var('#'), 'SET',
			$method(
				$method(
					$method($var('#'), 'value', $constant($null())),
					'length',
					$constant($null())
				),
				'asString',
...
$addVariable('main', $function($Array($String()), $String(), $functionBody(
	$sequence([
		...
		$assign('x', $constructor('Mutable', $tuple([$constant($type($String())), $var('y')]))),
		$assign('t', $constructor('Type', $constant($type($Real())))),
		$assign('e', $constructor('Error', $var('x'))),
		//$assign('x', $constant($mutable($String(), $string('initial')))),
		//$method($var('x'), 'SET', $var('y')),
		$call($var('mutate'), $var('x')),
		$method($tuple([$var('x'), $var('t'), $var('e')]), 'printed')
	])
)));
