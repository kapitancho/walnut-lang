...
$addVariable('fn', $function($Integer(), $Function($Integer(), $Integer()), $functionBody(
	$sequence([
		$assign('x', $var('#')),
		$constant($function($Integer(), $Integer(), $functionBody(
			$sequence([
				$assign('y', $var('#')),
				$method($var('x'), 'binaryPlus', $var('y'))
			])
		))),
	])
)));
...
$addVariable('main', $function($Array($String()), $String(), $functionBody(
	$sequence([
		$assign('x', $call($call($var('fn'), $constant($integer(42))), $constant($integer(-7)))),
