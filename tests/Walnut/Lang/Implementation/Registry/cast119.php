...
$addVariable('fn', $function($Any(), $Any(), $functionBody(
	$sequence([
		$assign('x', $constructor('DatabaseConnection', $record([
			'dsn' => $constant($string("sqlite:db.sqlite"))
		]))),
		$assign('y', $constructor('DatabaseConnector', $record(['connection' => $var('x')]))),
		$assign('z', $method($var('y'), 'execute', $record([
			'query' => $constant($string("CREATE TABLE IF NOT EXISTS projects (id integer PRIMARY KEY, name string)")),
			'boundParameters' => $constant($list([]))
		]))),

		$method($var('y'), 'execute', $record([
			'query' => $constant($string("INSERT INTO projects VALUES (1, 'Project 1')")),
			'boundParameters' => $constant($list([]))
		])),

		$method($var('y'), 'execute', $record([
			'query' => $constant($string("INSERT INTO projects VALUES (2, 'Project 22')")),
			'boundParameters' => $constant($list([]))
		])),

		$assign('w', $method($var('y'), 'execute', $record([
			'query' => $constant($string("CREATE TABLE projects (id integer PRIMARY KEY, name string)")),
			'boundParameters' => $constant($list([]))
		]))),

		$assign('v', $method($var('y'), 'query', $record([
			'query' => $constant($string("SELECT * FROM projects WHERE id <= 2")),
			'boundParameters' => $constant($list([]))
		]))),

		$method($var('y'), 'query', $record([
			'query' => $constant($string("SELECT wrong")),
			'boundParameters' => $constant($list([]))
		])),

		$tuple([$var('x'), $var('y'), $var('z'), $var('w'), $var('v')])
	])
)));

$addVariable('main', $function($Array($String()), $String(), $functionBody(
	$sequence([
		$assign('x', $call($var('fn'), $constant($null()))),
