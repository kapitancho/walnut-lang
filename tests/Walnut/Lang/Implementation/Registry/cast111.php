...
$addAlias('EvenNumberFinder', $Function($Integer(), $Boolean()));

$addAtom('IntegerOutpost');
...
$addMethod(
	...
	$Null(),
	null,
	$Alias('EvenNumberFinder'),
	$functionBody($constant($function($Integer(), $Boolean(), $functionBody(
		$method(
			$method($var('#'), 'binaryModulo', $constant($integer(2))),
			'binaryNotEqual',
			$constant($integer(1))
		),
	)))
...
$addVariable('check', $function($String(), $Result($Boolean(), $NamedType('NotANumber')), $functionBody(
	$call(
		$method($constant($atom('IntegerOutpost')), 'as', $constant($type($Alias('EvenNumberFinder')))),
		$noError($method($var('#'), 'as', $constant($type($Integer()))))
	)
)));
...
$addVariable('main', $function($Array($String()), $String(), $functionBody(
	$sequence([
		$assign('x',
			$matchType($var('#'), [
				$matchPair($constant($type($Tuple([$String()]))), $call($var('check'), $property($var('#'), '0'))),
				$matchDefault($constant($error($string("Please provide a number"))))
			])
		),
