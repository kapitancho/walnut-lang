...
$addVariable('petyaFn', $function(
	$String(), $String(), $functionBody(
		$method(
			$method($var('#'), 'length'),
			'as',
...
$addVariable('myFuncI', $function(
	$Tuple([$String(), $String()]), $Result($Integer(), $Atom('NotANumber')), $functionBody(
		$sequence([
			$assign('x', $property($var('#'), '0')),
			$assign('y', $property($var('#'), '1')),
			$method(
				$noError($method($var('x'), 'as', $constant($type($Integer())))),
				'binaryPlus',
				$noError($method($var('y'), 'as', $constant($type($Integer()))))
			)
		])
	)
));

$addVariable('myFunc', $function(
	$Array($String(), 2), $Result($Real(), $Union([$Atom('NotANumber'), $State('CastNotAvailable')])), $functionBody(
		$sequence([
			$assign('x', $method($var('#'), 'item', $constant($integer(0)))),
			$assign('y', $method($var('#'), 'item', $constant($integer(2)))),
			$method(
				$noError($method($var('x'), 'as', $constant($type($Real())))),
				'binaryPlus',
				$noError($method($var('y'), 'as', $constant($type($Real()))))
			)
		])
	)
));
...
$addVariable('main', $function($Array($String()), $String(), $functionBody(
	$sequence([
		$assign('x',
			$matchType($var('#'), [
				$matchPair($constant($type($Array($String(), 3))), $call($var('myFunc'), $var('#'))),
				$matchPair($constant($type($Tuple([$String(), $String()]))), $call($var('myFuncI'), $var('#'))),
				$matchPair($constant($type($Array($String(), 1, 1))),
					$call($var('petyaFn'), $method($var('#'), 'item', $constant($integer(0))))),
				$matchDefault($constant($string("Hi")))
			])
		),
