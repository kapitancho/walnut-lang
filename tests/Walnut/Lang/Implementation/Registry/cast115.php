...
$addVariable('innerScopeByType', $function($Any(), $Integer(), $functionBody(
	$matchType(
		$var('#'), [
			$matchPair($constant($type($Integer())), $var('#')),
			$matchDefault($constant($integer(1)))
		]
	)
)));

$addVariable('innerScopeByValue', $function($Any(), $Integer(), $functionBody(
	$matchValue(
		$var('#'), [
			$matchPair($constant($integer(3)), $var('#')),
			$matchDefault($constant($integer(2)))
		]
	)
)));
...
$addVariable('main', $function($Array($String()), $String(), $functionBody(
	$sequence([
		$assign('y',
			$matchType($var('#'), [
				$matchPair($constant($type($Tuple([$String()]))), $property($var('#'), '0')),
				$matchDefault($constant($string('null')))
			])
		),
		$assign('z', $method($var('y'), 'jsonDecode', $constant($null()))),
		$assign('x', $tuple([
			$call($var('innerScopeByType'), $var('z')),
			$call($var('innerScopeByValue'), $var('z')),
		])),
