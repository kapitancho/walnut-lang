...
$addAlias('PositiveInteger', $Integer(1));
$addVariable('fizzBuzz', $function(
	$Alias('PositiveInteger'), $String(), $functionBody(
		$matchTrue([
			$matchPair(
				$method(
					$method($var('#'), 'binaryModulo', $constant($integer(15))),
					'binaryEqual',
					$constant($integer(0))),
				$constant($string('fizzbuzz'))
			),
			$matchPair(
				$method(
					$method($var('#'), 'binaryModulo', $constant($integer(3))),
					'binaryEqual',
					$constant($integer(0))),
				$constant($string('fizz'))
			),
			$matchPair(
				$method(
					$method($var('#'), 'binaryModulo', $constant($integer(5))),
					'binaryEqual',
					$constant($integer(0))),
				$constant($string('buzz'))
			),
			$matchDefault($method($var('#'), 'as', $constant($type($String()))))
		])
	)
));

$addMethod($String(), 'asPositiveInteger', $Null(), null, $Result($Alias('PositiveInteger'), $Atom('NotANumber')), $functionBody(
	$sequence([
		$assign('x', $noError($method($var('$'), 'as', $constant($type($Integer()))))),
		$matchType(
			$var('x'), [
				$matchPair(
					$constant($type($Alias('PositiveInteger'))),
					$var('x')
				),
				$matchDefault($constant($error($atom('NotANumber'))))
			]
		)
	])
));

$addVariable('main', $function($Array($String()), $String(), $functionBody(
	$sequence([
		$assign('x',
			$matchType($var('#'), [
				$matchPair($constant($type($Tuple([$String()]))), $sequence([
					$assign('y', $method($property($var('#'), '0'), 'as', $constant($type($Alias('PositiveInteger'))))),
					$matchType($var('y'), [
						$matchPair(
							$constant($type($Alias('PositiveInteger'))),
							$call($var('fizzBuzz'), $var('y'))
						),
						$matchDefault($constant($error($string("Please provide a positive number"))))
					])
				])),
				$matchDefault($constant($error($string("Please provide just one argument"))))
			])
		),
		$method($var('x'), 'printed')
	])
)));
