...
$addAtom('NotAnEvenNumber');
$addSubtype('EvenNumber', $Integer(), $functionBody(
	$matchTrue([
		$matchPair(
			$method(
			...
			$constant($error($atom('NotAnEvenNumber')))
		),
		$matchDefault($constant($null()))
	])
), $Atom('NotAnEvenNumber'));

$addVariable('even', $function($Integer(), $Result($Subtype('EvenNumber'), $Atom('NotAnEvenNumber')), $functionBody(
	$constructor('EvenNumber', $var('#'))
)));

$addVariable('main', $function($Any(), $String(), $functionBody(
	$sequence([
		$assign('x', $tuple([
			$call($var('even'), $constant($integer(3))),
			$call($var('even'), $constant($integer(8)))
		])),
