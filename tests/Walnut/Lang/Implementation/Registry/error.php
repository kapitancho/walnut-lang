...
$emptyBody = $functionBody($sequence([]));

$addSubtype("Point", $Record([
	'x' => $Real(),
	'y' => $Real(),
]), $emptyBody, null);
$addSubtype("Point3d", $Tuple([$Real(), $Real(), $Real()]), $emptyBody, null);
$addSubtype("ProductId", $Integer(1, PlusInfinity::value), $emptyBody, null);

$addVariable('fn', $function($Record([
	'a' => $String(), 'b' => $Integer(0, PlusInfinity::value)
]), $String(), $functionBody(
	$method($property($var('#'), 'a'), 'substringRange', $record([
		'start' => $property($var('#'), 'b'),
		'end' => $method($property($var('#'), 'a'), 'length', $constant($null()))
	]))
)));
