...
$addVariable('defaultBookData', $dict([
...
$addAlias('ISBN', $String(13));

$addState('Book', $Record([
	'isbn' => $NamedType('ISBN'),
	'data' => $NamedType('BookData')
]));

$addState('BookX', $Record([
	'isbn' => $NamedType('ISBN'),
	'data' => $NamedType('BookData')
]));
	...
	$sequence([
		$assign('x', $tuple([
			$constructor('Book', $record([
				'isbn' => $constant($string('978-3-16-148410-0')),
				'data' => $record([
					'author' => $constant($string('John Doe')),
					'title' => $constant($string('My Book')),
					'year' => $constant($integer(2021)),
					'pages' => $constant($integer(123)),
					'price' => $constant($real(12.34))
				])
			])),
			$constructor('BookX', $constant($string('978-3-16-148410-0')))
		])),
