...
$addSubtype('EvenInteger', $Integer(), $functionBody(
	$constant($null())
), null);

$addEnumeration('Suit', ['Spades', 'Hearts', 'Diamonds', 'Clubs']);
...
$addMethod(
	...
	$functionBody(
		$matchValue(
			$var('#'), [
				$matchPair($constant($enumeration('Suit', 'Spades')), $constant($string('sp'))),
				$matchPair($constant($enumeration('Suit', 'Hearts')), $constant($string('he'))),
				$matchPair($constant($enumeration('Suit', 'Diamonds')), $constant($string('di'))),
				$matchDefault($constant($string('cl')))
			]
		)
	)
);
...
$addMethod(
	...
	null,
	$NamedType('JsonValue'),
	$functionBody($method($var('$'), 'as', $constant($type($String()))))
);

$addVariable('toJsonValue', $function($Any(), $Result($NamedType('JsonValue'), $NamedType('InvalidJsonValue')), $functionBody(
	$method($var('#'), 'asJsonValue', $constant($null()))
)));

$addVariable('main', $function($Array($String()), $String(), $functionBody(
	$sequence([
		$assign('x', $call($var('toJsonValue'), $tuple([
			$var('#'),
			$constructor('EvenInteger', $method($var('#'), 'length', $constant($null()))),
			$constant($enumeration('Suit', 'Clubs')),
			$method($constant($string("[1, true, null]")), 'jsonDecode', $constant($null())),
		]))),
